...
Stage 2: Extract
-----------------
Logs into the live WaiverPro app, crawls reachable authenticated routes,
and extracts structured UI data (DOM elements + screenshots) into the
canonical schema required by the assignment.
...
def perform_login(page):
    ...
    # The button shows a "Logging in..." loading state while the auth
    # request is in flight on a cold serverless backend - wait explicitly
    # for the URL to leave /login rather than relying on a fixed delay.
    try:
